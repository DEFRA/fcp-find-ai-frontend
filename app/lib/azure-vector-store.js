/**
 * This file was generated from https://github.com/langchain-ai/langchainjs/blob/51b7721967c95336e8f022e71d0d8e8ac39c0e21/libs/langchain-community/src/vectorstores/azure_aisearch.ts#L100
 * It is required as the langchain library doesn't currently allow us to override the field name within a document
 * AI Search expects the field to be 'content' but our field is named 'chunk'
 */
...
class AzureAISearchVectorStore extends vectorstores_1.VectorStore {
    ...
    constructor(embeddings, config) {
        ...
        if (!config.client) {
            ...
            credentials ?? (credentials = new search_documents_1.AzureKeyCredential(key));
            // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
    ...
    async similaritySearchWithScore(query, k = 4, filter = undefined) {
        const searchType = this.options.type ?? exports.AzureAISearchQueryType.Similarity;
    ...
    async similaritySearchVectorWithScore(query, k, filter) {
        ...
        for await (const item of results) {
            const document = new documents_1.Document({
                pageContent: item.document['chunk'],
                metadata: {
                    ...item.document[DEFAULT_FIELD_METADATA],
                },
            });
